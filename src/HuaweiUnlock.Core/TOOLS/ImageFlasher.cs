...
namespace HuaweiUnlocker.TOOLS
{
    public class ImageFlasher
    {
        ...
        private void SendHeadFrame(int length, int address)
        {
            ...
            // .NET 9: byte[].Reverse() binds to Span<T>.Reverse() (void); force LINQ.
            data.AddRange(Enumerable.Reverse(BitConverter.GetBytes(length)));
            data.AddRange(Enumerable.Reverse(BitConverter.GetBytes(address)));
